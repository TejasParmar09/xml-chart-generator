...
export default function FileDetailsPage() {
  ...
  useEffect(() => {
    const fetchFileDetails = async () => {
      ...
      if (!gridfsId || !/^[0-9a-fA-F]{24}$/.test(gridfsId)) {
        console.error('GridFS ID is missing or invalid from navigation state:', gridfsId);
        setError('GridFS ID is missing or invalid.'); // This is the error message
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const detailsResponse = await api.userFiles.getDetails(id);
        console.log('File details response:', detailsResponse.data);
        if (!detailsResponse.data) {
          throw new Error('No data returned from getDetails API.');
        }
        ...
        console.log(`Fetching file content for GridFS ID: ${gridfsId}`);
        const contentResponse = await api.userFiles.getContent(gridfsId);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      yPosition += 10;
      doc.text(`Username: ${username}`, 20, yPosition);
      yPosition += 10;
  ...
  if (!fileDetails) {
    return (
      <div className="bg-white rounded-xl shadow-lg p-6 mx-auto max-w-md text-center min-h-screen bg-gray-50">
        <Toaster position="top-right" />
        <h3 className="text-2xl font-bold text-gray-800 mb-4">File Not Found</h3>
        <p className="text-gray-700 mb-6">The requested file could not be found.</p>
        <button
          onClick={() => navigate('/user/files')}
          className="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-150 ease-in-out shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
        >
